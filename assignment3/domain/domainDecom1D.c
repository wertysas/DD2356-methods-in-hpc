...
int main(int argc, char *argv[]){
    ...
    MPI_Recv(&f[0], 1, MPI_DOUBLE, pos_mod(rank-1, size), 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);

    // Left Pass
    MPI_Send(&f[2], 1, MPI_DOUBLE, pos_mod(rank-1, size), 1, MPI_COMM_WORLD);
    ...
    // Check error the error |cos(x_i) - dfdx(i)| should be within O(dx^2) (central diff. quot.)
    double dx2 = dx*dx;
    for (i=1; i<(nxn_loc-1); i++) {
        double diff = fabs(dfdx[i] - (cos(L_loc*rank + (i-1) * dx)));
        if (diff > dx2) printf("rank: %d\ti: %d\tdiff: %f\n", rank, i, diff);
    }
